...
- Maintaining the public infrastructure (GitHub organization, npm scope,
  Docker registry namespace, the meetropolis.me domain).
...
in the GitHub team `@lass-machen/maintainers`.

### Lead maintainer

A specific maintainer who acts as the technical tie-breaker when consensus
cannot be reached. Currently **Dennis Menken** (`@dennismenken`). The
lead-maintainer role is held by a Tiamat employee for as long as Tiamat
funds the project; it is not transferred per-PR.
...
| Name          | GitHub          | Affiliation | Role            | Area                            |
| ------------- | --------------- | ----------- | --------------- | ------------------------------- |
| Dennis Menken | `@dennismenken` | Tiamat UG   | Lead maintainer | All areas, tiebreak on conflict |
| Rico Röder    | `@ricoroeder`   | Tiamat UG   | Maintainer      | All areas                       |
| Vincent Rothe | `@salzfass`     | Tiamat UG   | Maintainer      | All areas                       |
...
private advisory channel or `mail@meetropolis.me`. Coordinated
