...
export default tseslint.config(
  ...
  // 3c) Session-write boundary: every mutation of a `Session` row must go
  //     through apps/server/src/api/utils/sessionAuth.ts.
  //
  //     Authentication resolves a token against its `Session` row through a
  //     short-lived in-process cache (api/utils/sessionCache.ts). Deleting or
  //     shortening a row without invalidating that cache leaves the session
  //     usable for up to SESSION_CACHE_TTL_MS — REST *and* the Colyseus world
  //     join (rooms/lifecycle/onAuth.ts) resolve against the same cache, so a
  //     revoked guest could still enter the world with camera and microphone.
  //     That happened three times (guest revoke, guest-expiry middleware, room
  //     sweep) because the invalidation lived next to the delete instead of
  //     inside it. The helpers in sessionAuth.ts (revokeSessionById,
  //     revokeSessionsForUser, revokeSessionByToken, purgeExpiredSessions) pair
  //     each write with its invalidation; this rule is what keeps the next
  //     feature from re-opening the hole with a fresh `deleteMany`.
  //
  //     `create` is deliberately NOT restricted: a new row cannot make the
  //     cache stale (an uncached token is simply read from the database).
  {
    files: ['apps/server/src/**/*.ts'],
    ignores: ['apps/server/src/api/utils/sessionAuth.ts', 'apps/server/src/**/*.test.ts'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector:
            "CallExpression[callee.object.property.name='session'][callee.property.name=/^(delete|deleteMany|update|updateMany|upsert)$/]",
          message:
            'Session rows must not be mutated directly: the auth cache (api/utils/sessionCache.ts) would keep serving the revoked session. Use a helper from api/utils/sessionAuth.ts (revokeSessionsForUser, revokeSessionById, revokeSessionByToken, purgeExpiredSessions) — or add one there, next to its invalidation.',
        },
      ],
    },
  },
