...
// Periodic: clear pending clients that didn't resolve within the
// configured timeout. Returns the interval handle so onDispose can
// clear it.
export function startPendingCleanupInterval(
  room: WorldRoom,
  ...
  return setInterval(() => {
    ...
    for (const [identity, pending] of room.pendingClients.entries()) {
      if (now - pending.timestamp > pendingTimeoutMs) {
        room.pendingClients.delete(identity);
        try { pending.client.leave(1000); } catch { /* best-effort */ }
