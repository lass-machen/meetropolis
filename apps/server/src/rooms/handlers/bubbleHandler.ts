...
export function handleBubbleUpdate(room: WorldRoom, data: { id?: string; members?: string[] }): void {
  ...
  if (filtered.length >= 2) {
    const gid = data?.id && typeof data.id === 'string' && data.id.length > 0 ? data.id : canonicalGroupId(filtered);
