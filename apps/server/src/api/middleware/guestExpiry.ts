...
import { revokeSessionsForUser } from '../utils/sessionAuth.js';
...
export const guestExpiryMiddleware: express.RequestHandler = async (req, res, next) => {
  try {
    ...
    if (membership.expiresAt < new Date()) {
      // Guest is expired: kill sessions and block. Via the helper, never a raw
      // `session.deleteMany` — that leaves the cached resolution alive and the
      // expired guest keeps authenticating (including into the world, which
      // resolves against the same cache) until the TTL lapses.
      await revokeSessionsForUser(prisma, auth.userId);
