...
export function registerControlRoutes(
  ...
  app.post('/controls', async (req: express.Request, res: express.Response) => {
    ...
    // Restrictive-only: these broadcast paths may only carry protective
    // actions (disabling a device). Enabling mic/cam/share on a remote peer,
    // or setting their DND at all, is not a legitimate use case here, so
    // `false` is the only accepted value and `dnd` is rejected outright.
    // The single legitimate consumer is force-mute ({ mic: false }).
    const schema = z
      .object({
