...
import { avatarEditorEnabled } from '../../services/avatarComposer.js';

/**
 * Returns true when the optional enterprise billing module is loaded. The web
 * client uses this capability flag to skip `/billing/*` calls in OSS-only
 * deployments where those endpoints are not registered (and would otherwise
 * produce 404 console noise). All billing-secret handling (Stripe etc.) is
 * private to the EE module — the OSS server only sees this single boolean.
 */
...
export async function handleOssPublicConfig(_req: express.Request, res: express.Response): Promise<void> {
  ...
  const body: Record<string, unknown> = {
    ...
    billingEnabled,
    // Phase-2 character editor gate. Gates BOTH the endpoints and (via this
    // flag) the editor UI, so nothing is exposed before the Phase-3 decision.
    avatarEditorEnabled: avatarEditorEnabled(),
  };
  // Optional telemetry activation block, sourced from the closed-source telemetry
  // module. Absent in every OSS build (module not loaded → null), so the block is
  // omitted and no tracker hint ever leaks into an open-source deployment. When
  // present it carries only `{ enabled, environment }` — never any secret/key.
  // The wire key stays vendor-neutral (`telemetry`); the enterprise variant of
  // /public/config (billing/tenantAdminRoutes.ts) MUST emit the same key so the
  // browser telemetry module reads it in the enterprise deployment where the
  // enterprise handler wins over this OSS fallback.
  const telemetry = (await getTelemetryModule())?.getPublicConfigBlock() ?? null;
