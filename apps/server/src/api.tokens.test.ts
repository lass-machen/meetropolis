...
/** Session rows, keyed by token hash — see `signSession` below. */
type SessionRec = { id: string; userId: string; expiresAt: Date; lastActiveAt: Date };

const mem: { tokens: TokenRec[]; sessions: Map<string, SessionRec> } = { tokens: [], sessions: new Map() };

// Mock PrismaClient used inside api.ts. Note: api.ts imports from
// './generated/prisma/index.js' (the locally generated client), NOT from
// '@prisma/client', so the mock must target that exact specifier.
vi.mock('./generated/prisma/index.js', () => {
  class PrismaClientMock {
    ...
    // Consulted by the session-auth middleware on every request: a JWT
    // authenticates only while a Session row backs it (see sessionAuth.ts).
    session = {
      findUnique({ where }: any) {
        return (mem.sessions.get(where?.tokenHash) as any) ?? null;
      },
      update({ where }: any) {
        return (mem.sessions.get(where?.id) as any) ?? null;
      },
    };
  }
...
/** A session JWT plus the Session row that makes it authenticate. */
const signSession = (userId: string) => {
  const secret = process.env.JWT_SECRET!;
  const token = jwt.sign({}, secret, { subject: userId });
  mem.sessions.set(hashSessionToken(token), {
    id: `sess-${userId}`,
    userId,
