...
async function rebuildTilesetRegistry(mapId: string, tilesets: Tmj['tilesets']) {
...
async function clearExistingLayers(mapId: string) {
...
function buildTileRefArray(
  tmjLayer: NonNullable<Tmj['layers'][number]>,
  width: number,
  height: number,
  enc: 'rle' | 'rle-bool',
...
async function importLayers(mapId: string, tmj: Tmj, chunkSize: number, gidToTileRefId: (gid: number) => number) {
  ...
  for (const [layerName, enc] of layersWanted) {
    ...
    await persistChunks(layer.id, tileRefs, width, height, chunkSize, enc);
  }
}
...
async function main() {
  ...
  const map = await upsertMap(tenant.id, mapName, tmj, chunkSize);
  await rebuildTilesetRegistry(map.id, tmj.tilesets);
