...
export function useScreenshareEvents({
  ...
  React.useEffect(() => {
    ...
    const checkForNewScreenshares = () => {
      ...
      // Check local participant for screenshare
      if (localParticipant) {
        ...
        if (hasLocalScreen && !localScreenshareRef.current) {
          localScreenshareRef.current = true;
          onLocalScreenshareStart?.();
        } else if (!hasLocalScreen && localScreenshareRef.current) {
          localScreenshareRef.current = false;
          onLocalScreenshareStop?.();
        }
      }

      // Check remote participants for screenshare
      ...
      for (const remote of remotes) {
        ...
        if (hasScreen) {
          ...
          // New screenshare detected
          if (!knownScreensharesRef.current.has(p.sid)) {
      ...
      // Check for stopped screenshares
      for (const sid of knownScreensharesRef.current) {
        if (!currentScreenshares.has(sid)) {
          onRemoteScreenshareStop?.(sid);
        }
      }

      knownScreensharesRef.current = currentScreenshares;
    };
