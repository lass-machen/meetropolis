...
export function BGSpacer(props: { size?: ButtonGroupSpacerSize; className?: string; style?: React.CSSProperties }) {
  ...
  const sizeClass = size === 'grow' ? 'bg-spacer-grow' : size === 'shrink' ? 'bg-spacer-shrink' : size === 'fixedSm' ? 'bg-spacer-fixedSm' : size === 'fixedMd' ? 'bg-spacer-fixedMd' : size === 'fixedLg' ? 'bg-spacer-fixedLg' : 'bg-spacer-auto';
