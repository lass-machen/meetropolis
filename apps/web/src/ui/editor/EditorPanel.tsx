...
function buildPackItemsFromTileset(detail: any, category: 'terrain' | 'structures' | 'objects', img: HTMLImageElement): PackItem[] {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      items.push({ packUuid: 'local:tileset', itemId: `${detail.key}:${r}:${c}`, key: `${detail.key}-${r}-${c}`, category, dataUrl: url, width: detail.tileWidth, height: detail.tileHeight, collide: false });
    }
  }
  return items;
}
...
function useEditorEvents(setToast: Setter<ToastState>, setToastOpen: Setter<boolean>) {
  React.useEffect(() => {
    const onTilesetConfirm = (e: Event) => {
      ...
      const category = (detail.category === 'structures' || detail.category === 'objects' || detail.category === 'terrain') ? detail.category : 'terrain';
      const img = new Image();
      img.onload = () => {
...
function useV2Tilesets() {
  ...
  React.useEffect(() => {
    ...
    if (!mapId) return;
    fetchStateV2(mapId).then(v2 => { if (v2?.tilesetRegistry) setV2Tilesets(v2.tilesetRegistry); }).catch(() => {});
  }, []);
  return v2Tilesets;
}
    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'grid', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'spawn' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'spawn' ? 'rgba(59,130,246,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>Spawn setzen</button>
          <button onClick={() => EditorService.dispatch({ type: 'CLEAR_SPAWN' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>Entfernen</button>
      ...
      <div style={{ display: 'grid', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input type="color" value={state.backgroundColor || '#111827'} onChange={(e) => EditorService.dispatch({ type: 'SET_BACKGROUND_COLOR', color: e.target.value })} style={{ width: 32, height: 32, padding: 0, border: '1px solid var(--border)', borderRadius: 4, cursor: 'pointer', background: 'none' }} />
          <span style={{ fontSize: 12, color: 'var(--fg)', fontFamily: 'monospace' }}>{state.backgroundColor || '#111827'}</span>
        </div>
      </div>
    </div>
  );
...
function TerrainSettings({ state, v2Tilesets }: { state: ReturnType<typeof EditorService.getState>; v2Tilesets: V2Tileset[] }) {
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'grid', gap: 6 }}>
        <div style={{ fontSize: 12, color: 'var(--fg-subtle)' }}>Terrain Hintergrund</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input type="color" value={state.terrainColor || '#202020'} onChange={(e) => EditorService.dispatch({ type: 'SET_TERRAIN_COLOR', color: e.target.value })} style={{ width: 32, height: 32, padding: 0, border: '1px solid var(--border)', borderRadius: 4, cursor: 'pointer', background: 'none' }} />
          <span style={{ fontSize: 12, color: 'var(--fg)', fontFamily: 'monospace' }}>{state.terrainColor || '#202020'}</span>
        </div>
      </div>
      <TerrainTileGrid v2Tilesets={v2Tilesets} selectedTileRefId={state.selectedTileRefId} packTerrainItems={state.packItems.filter((it: PackItem) => it.category === 'terrain')} pendingAsset={state.pendingAsset} />
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', gap: 6 }}>
      {state.category === 'terrain' && (
        <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'terrain' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'terrain' ? 'rgba(59,130,246,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>Terrain</button>
      )}
      {state.category === 'collisions' && (
        <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'collision' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'collision' ? 'rgba(244,63,94,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>Kollision zeichnen</button>
      )}
      {state.category === 'autotiles' && (
        <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'wall' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'wall' ? 'rgba(139,90,43,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>Wand zeichnen</button>
      )}
      {state.category !== 'terrain' && state.category !== 'collisions' && state.category !== 'general' && state.category !== 'autotiles' && (
        <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'select' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'select' ? 'rgba(59,130,246,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>{t('editor.select')}</button>
      )}
      {state.category !== 'general' && (
        <button onClick={() => EditorService.dispatch({ type: 'SET_TOOL', tool: 'erase' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: state.tool === 'erase' ? 'rgba(239,68,68,0.18)' : 'var(--glass)', color: 'var(--fg)', fontSize: 13 }}>{t('editor.delete')}</button>
      )}
    </div>
  );
}
    ...
    <>
        ...
        {list.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(64px, 1fr))', gap: 8, maxHeight: 260, overflow: 'auto', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--glass)', padding: 8 }}>
            {list.map((item: PackItem) => {
              ...
              return (
                <button key={`${item.packUuid}:${item.itemId}`} onClick={() => EditorService.dispatch({ type: 'SELECT_ASSET', asset: item })} title={item.key} style={{ padding: 4, borderRadius: 8, border: `1px solid ${selected ? 'rgba(59,130,246,0.8)' : 'var(--border)'}`, background: selected ? 'rgba(59,130,246,0.12)' : 'var(--glass)', display: 'flex', alignItems: 'center', justifyContent: 'center', height: 64, cursor: 'pointer' }}>
      ...
      {state.pendingAsset?.rotationAllowed && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 }}>
          <button onClick={() => EditorService.dispatch({ type: 'ROTATE_PENDING_ASSET' })} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--glass)', color: 'var(--fg)', fontSize: 13, cursor: 'pointer' }}>Rotieren (R)</button>
          <span style={{ fontSize: 12, color: 'var(--fg-subtle)', fontFamily: 'monospace' }}>{state.pendingAsset.rotation ?? 0}°</span>
        </div>
      )}
    </>
  );
...
function FooterBar({ saving, lastSavedLabel, onSave, t }: { saving: boolean; lastSavedLabel: string | null; onSave: () => void; t: (k: string, opts?: any) => string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
      <div style={{ fontSize: 12, color: 'var(--fg-subtle)' }}>
        {lastSavedLabel ? t('editor.lastSaved', { time: lastSavedLabel }) : t('editor.notSaved')}
      </div>
      <button onClick={onSave} disabled={saving} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'rgba(59,130,246,0.18)', color: 'var(--fg)', fontSize: 13, fontWeight: 600, opacity: saving ? 0.7 : 1, cursor: saving ? 'default' : 'pointer' }}>
  ...
  return (
    <div style={{ padding: 16, display: 'grid', gap: 12 }}>
      ...
      {state.category === 'zones' && (
        <ZoneEditor zones={state.zones} zoneName={state.zoneName} editingZoneIndex={state.editingZoneIndex} tool={state.tool} onSave={props.onSave} />
      )}
      ...
      <FooterBar saving={saving} lastSavedLabel={lastSavedLabel} onSave={handleSaveClick} t={t} />
