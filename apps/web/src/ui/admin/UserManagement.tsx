...
function useUserMutations(baseUrl: string, t: (k: string) => string, ctx: { users: User[]; canChangeRoles: boolean; currentUserId: string | null; load: () => Promise<void>; setError: (e: string | null) => void }) {
  const { setError, load, users, canChangeRoles, currentUserId } = ctx;

  const changeRole = async (userId: string, newRole: 'admin' | 'member') => {
    try {
      setError(null);
      const res = await fetch(`${baseUrl}/users/${userId}/role`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ role: newRole }) });
...
function useUserManagement(baseUrl: string, t: (k: string) => string) {
  ...
  const mutations = useUserMutations(baseUrl, t, { users: loader.users, canChangeRoles, currentUserId: loader.currentUserId, load: loader.load, setError: loader.setError });

  React.useEffect(() => { loader.load(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
...
function RoleBadge({ role }: { role: 'owner' | 'admin' | 'member' | undefined }) {
  if (role === 'owner') {
    return (<span style={{ padding: '4px 10px', borderRadius: 12, background: 'rgba(234, 179, 8, 0.2)', color: '#fbbf24', fontSize: 12, fontWeight: 600 }}>Owner</span>);
  }
  return (
    <span style={{ padding: '4px 10px', borderRadius: 12, background: role === 'admin' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(107, 114, 128, 0.2)', color: role === 'admin' ? '#60a5fa' : '#9ca3af', fontSize: 12, fontWeight: 600 }}>
      {role === 'admin' ? 'Admin' : 'Member'}
    </span>
  );
}

function RoleCell({ user, edit, canChangeRoles, currentUserId, onChange }: { user: User; edit: EditUser | null; canChangeRoles: boolean; currentUserId: string | null; onChange: (id: string, role: 'admin' | 'member') => void }) {
  const role = (edit && edit.id === user.id) ? edit.role : user.role;
  if (role === 'owner') return <RoleBadge role="owner" />;
...
function DeleteCell({ userId, confirmDeleteId, setConfirmDeleteId, onDelete }: { userId: string; confirmDeleteId: string | null; setConfirmDeleteId: (s: string | null) => void; onDelete: (id: string) => void }) {
  const { t } = useTranslation();
  if (confirmDeleteId === userId) {
    return (
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <Button size="sm" variant="danger" onClick={() => onDelete(userId)}>{t('admin.users.confirmDelete')}</Button>
        <Button size="sm" onClick={() => setConfirmDeleteId(null)}>&#x2715;</Button>
      </div>
    );
  }
  return <Button size="sm" variant="danger" onClick={() => setConfirmDeleteId(userId)}>{t('admin.users.delete')}</Button>;
}

function EditRow({ edit, setEdit, canChangeRoles, currentUserId, onSave }: { edit: EditUser; setEdit: (u: EditUser | null) => void; canChangeRoles: boolean; currentUserId: string | null; onSave: (u: EditUser) => void }) {
  return (
    <>
      <Td style={{ paddingLeft: 0 }}>
        <Input value={edit.email} onChange={e => setEdit({ ...edit, email: e.target.value })} style={{ padding: '8px 12px', fontSize: 14 }} />
      </Td>
      <Td>
        <Input value={edit.name ?? ''} onChange={e => setEdit({ ...edit, name: e.target.value })} style={{ padding: '8px 12px', fontSize: 14 }} />
      </Td>
      <Td>
        {edit.role === 'owner' ? <RoleBadge role="owner" /> : canChangeRoles && edit.id !== currentUserId ? (
          <Select value={edit.role || 'member'} onChange={(val) => setEdit({ ...edit, role: val as Role })} style={{ width: 'auto' }} options={[{ value: 'admin', label: 'Admin' }, { value: 'member', label: 'Member' }]} />
        ) : <RoleBadge role={edit.role} />}
      </Td>
      <Td style={{ paddingRight: 0, textAlign: 'right', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <Button size="sm" variant="brand" onClick={() => onSave(edit)}>✓</Button>
        <Button size="sm" onClick={() => setEdit(null)}>✕</Button>
      </Td>
    </>
  );
}

function ViewRow({ u, edit, setEdit, canChangeRoles, currentUserId, changeRole, baseUrl, setError, openReset, confirmDeleteId, setConfirmDeleteId, onDelete }: { u: User; edit: EditUser | null; setEdit: (e: EditUser | null) => void; canChangeRoles: boolean; currentUserId: string | null; changeRole: (id: string, role: 'admin' | 'member') => void; baseUrl: string; setError: (e: string | null) => void; openReset: (d: { for: { id: string; email: string }; token: string | null }) => void; confirmDeleteId: string | null; setConfirmDeleteId: (s: string | null) => void; onDelete: (id: string) => void }) {
  const { t } = useTranslation();
  return (
    <>
      ...
      <Td>
        <RoleCell user={u} edit={edit} canChangeRoles={canChangeRoles} currentUserId={currentUserId} onChange={changeRole} />
      </Td>
      <Td style={{ paddingRight: 0, textAlign: 'right', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <Button size="sm" onClick={() => setEdit({ id: u.id, email: u.email, name: u.name ?? '', role: u.role })}>{t('admin.users.edit')}</Button>
        <ResetTokenButton user={u} baseUrl={baseUrl} setError={setError} openReset={openReset} />
        <DeleteCell userId={u.id} confirmDeleteId={confirmDeleteId} setConfirmDeleteId={setConfirmDeleteId} onDelete={onDelete} />
      </Td>
    </>
  );
}

function UsersTableBody({ loading, users, edit, setEdit, canChangeRoles, currentUserId, changeRole, save, baseUrl, setError, openReset, confirmDeleteId, setConfirmDeleteId, onDelete }: { loading: boolean; users: User[]; edit: EditUser | null; setEdit: (e: EditUser | null) => void; canChangeRoles: boolean; currentUserId: string | null; changeRole: (id: string, r: 'admin' | 'member') => void; save: (u: EditUser) => Promise<boolean>; baseUrl: string; setError: (e: string | null) => void; openReset: (d: { for: { id: string; email: string }; token: string | null }) => void; confirmDeleteId: string | null; setConfirmDeleteId: (s: string | null) => void; onDelete: (id: string) => void }) {
  const { t } = useTranslation();
  ...
  if (users.length === 0) {
    return (
      <TBody>
        <Tr>
          <Td colSpan={4} style={{ paddingLeft: 0, textAlign: 'center', color: 'var(--fg-subtle)', padding: '32px 0' }}>{t('admin.users.none')}</Td>
        </Tr>
      </TBody>
    );
  }
    ...
    <TBody>
      {users.map(u => (
        <Tr key={u.id} style={{ borderBottom: '1px solid var(--border)', background: edit?.id === u.id ? 'rgba(59,130,246,0.1)' : 'transparent' }}>
          {edit?.id === u.id ? (
            <EditRow edit={edit} setEdit={setEdit} canChangeRoles={canChangeRoles} currentUserId={currentUserId} onSave={handleSave} />
          ) : (
            <ViewRow u={u} edit={edit} setEdit={setEdit} canChangeRoles={canChangeRoles} currentUserId={currentUserId} changeRole={changeRole} baseUrl={baseUrl} setError={setError} openReset={openReset} confirmDeleteId={confirmDeleteId} setConfirmDeleteId={setConfirmDeleteId} onDelete={onDelete} />
          )}
        </Tr>
      ))}
    </TBody>
...
function CreateUserModal({ open, onOpenChange, baseUrl, isOwner, setError }: { open: boolean; onOpenChange: (v: boolean) => void; baseUrl: string; isOwner: boolean; setError: (e: string | null) => void }) {
  const { t } = useTranslation();
  ...
  React.useEffect(() => { if (open) { setInviteCode(null); setNewEmail(''); setNewName(''); } }, [open]);

  const handleInvite = async () => {
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/auth/invite`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ email: newEmail, name: newName || undefined, role: newRole }) });
      if (!res.ok) throw new Error(translateApiError((await res.json())?.error) || t('common.error'));
      const data = await res.json();
  ...
  return (
    <Modal zIndexBase={1100} open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) setNewRole('member'); }} title={t('admin.users.inviteTitle')} maxWidth={520} footer={<>
      <Button onClick={() => onOpenChange(false)}>{t('admin.users.cancel')}</Button>
      <Button variant="brand" onClick={handleInvite}>{t('admin.users.createInvite')}</Button>
    </>}>
      <div style={{ display: 'grid', gap: 10 }}>
        ...
        {isOwner && (
          <div style={{ display: 'grid', gap: 4 }}>
            ...
            <Select value={newRole} onChange={(val) => setNewRole(val as 'admin' | 'member')} options={[{ value: 'member', label: 'Member' }, { value: 'admin', label: 'Admin' }]} />
            <div style={{ fontSize: 11, color: 'var(--fg-subtle)' }}>Admins können weitere Member einladen und User verwalten.</div>
          </div>
        )}
        {inviteCode && (
          <div className="glass-surface" style={{ padding: 12, borderRadius: 'var(--radius-sm)', display: 'grid', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ flex: 1, padding: '10px 12px', borderRadius: 'var(--radius-xs)', border: '1px solid var(--border)', background: 'var(--glass)', color: 'var(--fg)', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontWeight: 700, letterSpacing: '0.06em' }}>{inviteCode}</div>
...
export function UserManagement(props: { baseUrl: string; onBack: () => void }) {
  ...
  const { t } = useTranslation();
  const { loading, users, error, setError, currentUserId, canChangeRoles, isOwner, changeRole, save, remove } = useUserManagement(baseUrl, t);
    ...
    <div style={{ width: '100%', display: 'grid', gap: 10 }}>
      <Toolbar
        left={<>
          ...
          <div style={{ padding: '6px 12px', borderRadius: 20, background: 'var(--glass)', border: '1px solid var(--border)', fontSize: 12, color: 'var(--fg)', fontWeight: 600 }}>{t('admin.users.adminBadge')}</div>
        </>}
        right={<><Button variant="brand" onClick={() => setCreateOpen(true)}>+ {t('admin.users.newUser')}</Button></>}
