...
function ErrorBanner({ error, onDismiss }: { error: string; onDismiss: () => void }) {
  return (
    <div style={{ padding: '8px 12px', borderRadius: 8, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#fca5a5', fontSize: 13, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <span>{error}</span>
      <button onClick={onDismiss} style={{ background: 'none', border: 'none', color: '#fca5a5', cursor: 'pointer', fontSize: 16 }}>&#x2715;</button>
    </div>
  );
}

function TokenRow({ token, t, onDelete }: { token: ApiToken; t: (k: string) => string; onDelete: () => Promise<void> }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid var(--border)', borderRadius:8, padding:'8px 10px', background:'var(--glass)' }}>
      <div>
      ...
      <Button variant="danger" onClick={onDelete} style={{ padding:'6px 8px' }}>{t('admin.api.delete')}</Button>
    </div>
  );
}
    ...
    <Modal open={open} onOpenChange={(o)=>{ if(!o) onClose(); }} title={t('admin.api.title')}>
      <div style={{ display:'grid', gap: 10 }}>
        ...
        <div style={{ display:'flex', gap: 12, alignItems:'center' }}>
          ...
          <Button variant="brand" onClick={createToken}>{t('admin.api.createToken')}</Button>
        </div>
        {freshToken && (
