  ...
  return (
    <div style={{ width: 240, height: '100%', background: 'rgba(15,15,18,0.82)', borderLeft: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        {online.length > 0 && (
          <div>
            <div style={{ padding: '6px 12px', fontSize: 11, color: 'var(--fg-subtle)', letterSpacing: 0.4, textTransform: 'uppercase' }}>{t('roster.onlineHeader')}</div>
            {online.map(r => (
        ...
        {offline.length > 0 && (
          <div>
            <div style={{ padding: '6px 12px', fontSize: 11, color: 'var(--fg-subtle)', letterSpacing: 0.4, textTransform: 'uppercase' }}>{t('roster.offlineHeader')}</div>
            {offline.map(r => (
