...
export function ExpandedCard({
  ...
  return (
    <div
      ...
      style={{
        // Container-relative sizing: the overlay stage shrinks with its grid
        // column (e.g. when the roster panel expands), so the card must size
        // against the stage, never against viewport units.
        width: '100%',
        maxWidth: full ? 1920 : 200,
    ...
    >
      <video
        ...
        style={{
          ...
          // Pan translates the video content inside the clipped card, so
          // panning reveals off-screen regions of the zoomed content.
          transform: `translate(${pan.x}px, ${pan.y}px) ${
            isLocal && part.media === 'camera' ? 'scaleX(-1) ' : ''
          }scale(${zoom})`,
