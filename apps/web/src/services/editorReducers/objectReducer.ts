...
function applyPendingObjectDelete(state: EditorState, action: Extract<EditorAction, { type: 'ADD_PENDING_OBJECT_DELETE' }>): Partial<EditorState> {
  ...
  if (objToDelete?._pending === 'add') {
    ...
    updatedPending = { ...updatedPending, objectsToAdd: updatedPending.objectsToAdd.filter(o => String(o.id) !== String(action.objectId)) };
  ...
  updatedPending = { ...updatedPending, objectUpdates: updatedPending.objectUpdates.filter(u => String(u.id) !== String(action.objectId)) };
  return { mapObjects: updatedMapObjects, pendingChanges: updatedPending, assets: updatedAssets };
}

function applyPendingObjectUpdate(state: EditorState, action: Extract<EditorAction, { type: 'ADD_PENDING_OBJECT_UPDATE' }>): Partial<EditorState> {
...
export function reduceObject(state: EditorState, action: ObjectActions): Partial<EditorState> | null {
  switch (action.type) {
    ...
    case 'UPDATE_MAP_OBJECT': {
      const objects = state.mapObjects.map(o => (String(o.id) === String(action.objectId)) ? { ...o, ...action.updates } : o);
      return { mapObjects: objects };
    }
    case 'ADD_PENDING_TERRAIN_PAINT':
      return { pendingChanges: { ...state.pendingChanges, terrainPaints: [...state.pendingChanges.terrainPaints, action.paint] } };
    case 'ADD_PENDING_OBJECT_CREATE':
      return {
        mapObjects: [...state.mapObjects, action.object],
        pendingChanges: { ...state.pendingChanges, objectsToAdd: [...state.pendingChanges.objectsToAdd, action.object] },
      };
