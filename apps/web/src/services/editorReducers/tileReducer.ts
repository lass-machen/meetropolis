 ...
 */
export function reduceTile(state: EditorState, action: TileActions): { update: Partial<EditorState>; followups?: CompleteTileFollowups } {
  switch (action.type) {
    case 'START_TILE_DRAG':
      return { update: { dragState: { startTileX: action.tileX, startTileY: action.tileY, endTileX: action.tileX, endTileY: action.tileY, tileDragMode: action.mode } } };
...
function computeCompleteTileDrag(state: EditorState, action: Extract<EditorAction, { type: 'COMPLETE_TILE_DRAG' }>): { update: Partial<EditorState>; followups: CompleteTileFollowups } {
  ...
  switch (tileDragMode) {
    case 'terrain':
      followups.push({ type: 'ADD_PENDING_TERRAIN_PAINT', paint: { layer: 'ground', rect, tileRefId: state.selectedTileRefId } });
      break;
    case 'wall':
      followups.push({ type: 'ADD_PENDING_TERRAIN_PAINT', paint: { layer: 'walls_auto', rect, tileRefId: state.selectedWallTypeId } });
      break;
    case 'collision':
    ...
    case 'erase': {
      ...
      if (cat === 'terrain' || cat === 'autotiles') {
        ...
        followups.push({ type: 'ADD_PENDING_TERRAIN_PAINT', paint: { layer: 'walls', rect, tileRefId: 0, erase: true } });
      } else if (cat === 'collisions') {
        followups.push({ type: 'ADD_PENDING_TERRAIN_PAINT', paint: { layer: 'collision', rect, tileRefId: 0, erase: true } });
      }
      break;
    }
  }
