 ...
 *
 * Reducers für einzelne Domains liegen in `editorReducers/` und werden
 * vom Service-Singleton zusammengezogen, sobald die jeweilige Action
 * eintrifft.
 */
...
const ZONE_ACTIONS = new Set<EditorAction['type']>([
  'START_ZONE_DRAG', 'UPDATE_ZONE_DRAG', 'COMPLETE_ZONE', 'DELETE_ZONE', 'START_EDIT_ZONE',
  'UPDATE_ZONE_NAME', 'SET_ZONE_NAME', 'UPDATE_ZONE_TYPE', 'UPDATE_ZONE_PORTAL', 'MARK_ZONES_MODIFIED',
]);

const ASSET_ACTIONS = new Set<EditorAction['type']>([
  'SELECT_ASSET', 'PLACE_ASSET', 'START_ASSET_DRAG', 'UPDATE_ASSET_DRAG', 'COMPLETE_ASSET_DRAG',
  'DELETE_ASSET', 'ADD_PACK_ITEMS', 'REGISTER_TILESET', 'LOAD_TILESETS', 'ROTATE_PENDING_ASSET',
  'SELECT_WALL_TYPE', 'SET_AUTOTILE_ITEMS', 'SELECT_TILE_REF',
]);

const OBJECT_ACTIONS = new Set<EditorAction['type']>([
  'SELECT_MAP_OBJECT', 'UPDATE_MAP_OBJECT', 'ADD_PENDING_TERRAIN_PAINT', 'ADD_PENDING_OBJECT_CREATE',
  'ADD_PENDING_OBJECT_DELETE', 'REMOVE_PENDING_OBJECT_DELETE', 'ADD_PENDING_OBJECT_UPDATE',
  'SET_PENDING_SPAWN', 'CLEAR_PENDING_CHANGES', 'LOAD_MAP_OBJECTS',
]);

const TILE_ACTIONS = new Set<EditorAction['type']>([
  'START_TILE_DRAG', 'UPDATE_TILE_DRAG', 'COMPLETE_TILE_DRAG',
]);
...
function reduceCore(state: EditorState, action: EditorAction): Partial<EditorState> | null {
  switch (action.type) {
    case 'ACTIVATE_EDITOR':
      return { active: true, category: action.category || 'terrain', tool: action.category === 'zones' ? 'zone' : 'select' };
...
class EditorServiceClass {
  ...
  public dispatch(action: EditorAction): void {
    ...
    // Exhaustive — type system already enforces, but guard at runtime too.
