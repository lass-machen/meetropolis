  ...
  findFreeSpotNear(
    ...
    for (let ring = 1; ring <= maxRings; ring++) {
      ...
      for (let angle = 0; angle < 360; angle += 30) {
        ...
        const tx = Math.round((target.x + Math.cos(rad) * r) / this.mapRef.tileWidth) *
                   this.mapRef.tileWidth + this.mapRef.tileWidth / 2;
        const ty = Math.round((target.y + Math.sin(rad) * r) / this.mapRef.tileHeight) *
                   this.mapRef.tileHeight + this.mapRef.tileHeight / 2;
        if (this.isWalkable(tx, ty)) return { x: tx, y: ty };
      }

      const dirs = [[r, 0], [-r, 0], [0, r], [0, -r]];
      for (const [dx, dy] of dirs) {
        const tx = Math.round((target.x + dx) / this.mapRef.tileWidth) *
                   this.mapRef.tileWidth + this.mapRef.tileWidth / 2;
        const ty = Math.round((target.y + dy) / this.mapRef.tileHeight) *
                   this.mapRef.tileHeight + this.mapRef.tileHeight / 2;
        if (this.isWalkable(tx, ty)) return { x: tx, y: ty };
      }
    }
