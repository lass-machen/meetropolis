...
export class MainScene extends Phaser.Scene implements MainSceneShape {
  ...
  private initializeManagers() {
    ...
    const avatarId = localStorage.getItem('avatarId') || 'default-characters:business_man';
    ...
    // A stored custom avatar has no preloaded texture/manifest at boot (it is
    // not part of the default packs), so the hero would render the default.
    // Resolve + swap it in asynchronously so the hero shows its own custom look.
    if (avatarId.startsWith('custom:') && !this.textures.exists(avatarRegistry.getTextureKey(avatarId))) {
      this.changeHeroAvatar(avatarId);
    }
  ...
  changeHeroAvatar(avatarId: string) {
    ...
    // Custom avatars (Phase 2 editor) need their manifest resolved before the
    // texture can be preloaded; resolve first (deduped) and re-enter on arrival.
    if (!avatarRegistry.getManifest(avatarId)) {
      void avatarRegistry.ensureManifest(avatarId).then((manifest) => {
        if (manifest) this.changeHeroAvatar(avatarId);
      });
      return;
    }
