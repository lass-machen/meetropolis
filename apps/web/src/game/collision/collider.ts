...
export function ensureCollisionCollider(scene: MainSceneLike): void {
  try {
    if (!scene.collisionLayer || !scene.hero) return;
    try {
      scene.collisionLayer.setCollisionByExclusion([-1], true);
    } catch {}
    try {
      scene.heroVsTilesCollider?.destroy();
    } catch {}
    scene.heroVsTilesCollider = scene.physics.add.collider(scene.hero, scene.collisionLayer);
  } catch {}
}

export function rebuildStaticColliders(scene: MainSceneLike): void {
  try {
    ...
    if (scene.hero) {
      ...
      const existing = scene.heroVsStaticCollider;
