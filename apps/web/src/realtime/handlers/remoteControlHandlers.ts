...
function showForceMutedToast(): Promise<void> {
  return import('../../lib/i18n').then(({ default: i18n }) => {
...
async function applyMicControl(target: boolean, args: UseWorldRoomArgs): Promise<void> {
  ...
  if (!target) {
    try { await showForceMutedToast(); } catch {}
  }
}
...
async function applyDndControl(target: boolean, args: UseWorldRoomArgs): Promise<void> {
  ...
  try { await avRef.current?.setDoNotDisturb(target); } catch {}
  if (gameBridge && typeof (gameBridge as any).setDoNotDisturb === 'function') (gameBridge as any).setDoNotDisturb(target);
  if (target) {
  ...
  setAvState(s => ({ ...s, dnd: target, mic: target ? false : s.mic, cam: target ? false : s.cam, share: target ? false : s.share }));
  dndRef.current = target;
  try { colyseusRef.current?.send?.('dnd_status', { dnd: target }); } catch {}
  ...
  setTimeout(async () => {
    try {
      ...
      setAvState(s => ({ ...s, mic: target ? false : realMic, cam: target ? false : realCam, share: target ? false : realShare }));
    } catch {}
  }, 450);
}
...
async function applyRemoteControl(payload: RemoteControlPayload, args: UseWorldRoomArgs): Promise<void> {
  if (typeof payload.mic === 'boolean') {
    try { await applyMicControl(!!payload.mic, args); } catch {}
  }
  if (typeof payload.cam === 'boolean') {
    try { await applyCamControl(!!payload.cam, args); } catch {}
  }
  if (typeof payload.share === 'boolean') {
    try { await applyShareControl(!!payload.share, args); } catch {}
  }
