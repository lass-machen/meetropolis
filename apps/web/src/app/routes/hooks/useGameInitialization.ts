...
function buildSavePosition(params: UseGameInitializationParams) {
  ...
  return async (opts?: { immediate?: boolean }) => {
    ...
    try {
      if (opts?.immediate) {
        fetch(`${apiBase}/auth/position`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, keepalive: true, body: payload }).catch(() => {});
      } else {
  ...
  if (!bubblePendingRef.current) {
    if (f.following) {
      ...
      if (target) {
        ...
        if (dist <= 12) { manualNavRef.current = null; gameBridge.setDesiredPosition(null); }
...
function buildLocalMoveHandler(params: UseGameInitializationParams, savePosition: (opts?: { immediate?: boolean }) => Promise<void>) {
...
async function initPhaserGame(params: UseGameInitializationParams) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { try { gameBridge.setHeroName(heroName); } catch (e) { logger.debug('[WorldApp] Operation failed', e); } }, 100);
  }, 0);
  bindGameBridgeHandlers(params);
  return game;
}

function buildCleanup(params: UseGameInitializationParams, game: ReturnType<typeof createPhaserGame> | null, handleBeforeUnload: () => void) {
...
export function useGameInitialization(params: UseGameInitializationParams) {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => { savePosition({ immediate: true }); };
    ...
    initPhaserGame(params).then(g => { game = g; });
