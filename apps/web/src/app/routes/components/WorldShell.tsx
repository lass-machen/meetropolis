...
function MainContent(props: WorldShellProps) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      />
      {/* Native "Preferences" (Cmd+,) opens the unified AV settings dialog via
          the AV toolbar; there is no separate desktop preferences modal. */}
      {/* UpdateBanner now mounts at the routing root (DesktopUpdateOverlay) so
          it is reachable from the login screen; do not re-add it here. */}
    </div>
  );
}
