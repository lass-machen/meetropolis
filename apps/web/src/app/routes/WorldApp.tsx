...
function useAuthState() {
  ...
  return { authChecked, setAuthChecked, me, setMe, isInternalOwner, setIsInternalOwner, capabilities, setCapabilities, authRefetchTrigger, setAuthRefetchTrigger, positionReady, setPositionReady, billingAvailable, setBillingAvailable };
}
...
function useDisposedFlag(disposedRef: React.MutableRefObject<boolean>) {
  useEffect(() => { disposedRef.current = false; return () => { disposedRef.current = true; }; }, [disposedRef]);
}
...
function useEscapeHandlers(setContextMenu: React.Dispatch<React.SetStateAction<any>>, selectedSid: string | null, setSelectedSid: React.Dispatch<React.SetStateAction<string | null>>, setOverlayZoom: React.Dispatch<React.SetStateAction<number>>) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setContextMenu({ open: false, x: 0, y: 0, playerId: null }); };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [setContextMenu]);
  useEffect(() => {
    if (!selectedSid) return;
    const handleEscape = (e: KeyboardEvent) => { if (e.key === 'Escape') { e.preventDefault(); setSelectedSid(null); setOverlayZoom(1); } };
...
async function saveAllToServerImpl(apiBase: string, editor: any, colyseusRef: React.RefObject<any>) {
  try {
    ...
    const res = await fetch(`${apiBase}/maps/${encodeURIComponent(mapId)}/editor-state`, { method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (!res.ok) {
...
export function WorldApp() {
  ...
  const core = useWorldAppCore({
    refs, auth, ui, apiBase,
    useBillingAvailability, useCameraManualSync, useDisposedFlag,
    useAvailableMaps, useEscapeHandlers, saveAllToServerImpl,
  });
  ...
  return (
    <WorldShell
      ...
      toggleMiniMode={core.desktop.toggleMiniMode}
      ...
      handleAuthComplete={core.handleAuthComplete}
