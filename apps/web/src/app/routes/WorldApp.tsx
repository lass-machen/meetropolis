...
function useWorldRefs() {
  // Read once: a ref only keeps its first argument, but the expression would
  // otherwise hit localStorage on every render.
  const initialDnd = React.useMemo(readPersistedDnd, []);
  return {
    ...
    // Seeded from storage so the Colyseus join re-asserts a persisted DND right
    // away (see dndPersistence.ts); otherwise peers would briefly see the user
    // as available after a reload.
    dndRef: useRef<boolean>(initialDnd),
...
function useAuthState() {
  ...
  const [authOffline, setAuthOffline] = React.useState(false);
...
function useUiPanels() {
  ...
  const [avState, setAvState] = React.useState<{
    mic: boolean;
    cam: boolean;
    share: boolean;
    dnd: boolean;
    ...
    cam: false,
    share: false,
    // Lazy: WorldApp re-renders several times per second, and only the first
    // value is used.
    dnd: readPersistedDnd(),
  }));
