...
// av_zone_permissions push (see
// apps/server/src/rooms/audioZones/permissionOrchestrator.ts). Sent only
// to the affected identity's own client: `allow` lists every other
// LiveKit identity currently allowed to subscribe to this client's own
// published tracks. The client applies it via
// `localParticipant.setTrackSubscriptionPermissions` (see
// apps/web/src/av/manager/zonePermissionsManager.ts) - the SFU-hard H4
// audio-zone privacy boundary.
export interface ZonePermissionsMessage {
  islandId?: string;
  allow?: string[];
}
