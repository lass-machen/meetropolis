...
type TilesetImageToLoad = { key: string; url: string; tileWidth: number; tileHeight: number };

/**
 * Registers a generated texture for a tileset image that could not be
 * loaded. The map load chain must keep going with the remaining tilesets;
 * an existing texture key also stops repeated download attempts on the
 * next preload run.
 */
function createPlaceholderTexture(scene: Phaser.Scene, def: TilesetImageToLoad): void {
  try {
    if (scene.textures.exists(def.key)) return;
    const width = Math.max(1, def.tileWidth);
    const height = Math.max(1, def.tileHeight);
    const canvasTexture = scene.textures.createCanvas(def.key, width, height);
    ...
    logger.warn(`[MapV2] Using generated placeholder texture for tileset "${def.key}"`);
  } catch (e) {
    logger.error(`[MapV2] Failed to create placeholder texture for tileset "${def.key}"`, e);
  }
}

export async function preloadTilesetImages(scene: Phaser.Scene, tilesets: V2Tileset[]): Promise<void> {
  const toLoad: TilesetImageToLoad[] = [];

  for (const ts of tilesets) {
    if (!scene.textures.exists(ts.key)) {
      toLoad.push({ key: ts.key, url: ts.imageUrl, tileWidth: ts.tileWidth, tileHeight: ts.tileHeight });
    }
  }
  if (toLoad.length === 0) return;

  const byKey = new Map(toLoad.map((def) => [def.key, def]));
  ...
  await new Promise<void>((resolve) => {
    const onLoadError = (file: { key?: string }) => {
      ...
      failedKeys.add(key);
      logger.warn(`[MapV2] Tileset image failed to load: "${key}"; continuing without it`);
    };
    const onComplete = () => {
      cleanup();
      resolve();
  ...
  for (const key of failedKeys) {
    ...
    if (def) createPlaceholderTexture(scene, def);
  }
}
