...
export default defineConfig({
  ...
  build: {
    ...
    minify: 'esbuild',
    target: 'es2020',
    // Never inline AudioWorklet modules. They are loaded through
    // `audioContext.audioWorklet.addModule(url)`, and a module URL is governed
    // by the `script-src` CSP directive — an inlined one becomes a `data:` URL,
    // which `script-src 'self'` refuses. The noise-gate worklet sits just under
    // Vite's 4096-byte inline threshold, so it was silently embedded and the
    // desktop build (the only one with a CSP) could not publish a microphone
    // track at all: voice isolation threw, and the whole mic publish failed
    // with it. Keeping the worklet a real asset keeps `script-src 'self'`
    // sufficient, instead of weakening the policy with `data:`.
    assetsInlineLimit: (filePath: string) => (filePath.includes('/worklets/') ? false : undefined),
